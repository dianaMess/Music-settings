...
void equalizer(vector<Complex> sound, double factor, double sec, vector<char> buf) {
    ...
    while (size > 0) {
        ...
        int n = spectr2.size();
        for (int i = 0; i < n / 2; i++) {
            vec[i] = spectr2[i] * (factor * (i - spectr2[(spectr2.size() / 2 - 1) / 2].re()) + 1);
            vec[n - 1 - i] = vec[i];
        }
